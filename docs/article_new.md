...
В собранном сайте не должны использоваться [React](https://reactjs.org/), [Vue.js](https://vuejs.org/), [Angular](https://angularjs.org/). Причина заключается не в том, что я не люблю эти фреймворки, а я хочу универсальный подход использовать без привязки в JavaScript фреймворку.
