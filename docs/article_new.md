...
В собранном сайте не должны использоваться React, Vue.js. Причина заключается не в том, что я не люблю эти фреймворки, а я хочу универсальный подход использовать без привязки в JavaScript фреймворку.
