...
Спорным решением может показаться разделение изображений на две директории: `img` и `uploads`. Но здесь использовал идеологию расположения файлов из [Wordpress](https://ru.wordpress.org/download/). На мой взгляд, кидать все изображения в одну директорию — это не очень хорошая идея.

В директории проекта [static-site-webpack-habr](https://github.com/Harrix/static-site-webpack-habr) находятся также дополнительные служебные файлы и директории (например, директория `docs` с документацией к проекту, `README.md` и др.). Но эти все остальные файлы и директории несут лишь служебную функцию, и можно копировать, рассматривать только вышеприведенные.
