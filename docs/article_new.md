...
Спорным решением может показаться разделение изображений на две директории: `img` и `uploads`. Но здесь использовал идеологию расположения файлов из [Wordpress](https://ru.wordpress.org/download/). На мой взгляд, кидать все изображения в одну директорию — не очень хорошая идея.
